...
class DoffinApiClient {
  ...
  private async makeRequest<T>(
    ...
    try {
      const headers: Record<string, string> = {
        ...
        'User-Agent': 'DoffinMCPServer/2.0',
      };
  ...
  private async downloadFile(url: string): Promise<{ content: string; contentType: string }> {
  ...
  async downloadDocumentFile(url: string): Promise<{ content: string; contentType: string }> {
...
class DoffinMcpServer {
  ...
  private getTools(): Tool[] {
    return [
      ...
      },
      {
        ...
        description: '[EXPERIMENTAL] Download all documents attached to a procurement notice. This tool fetches the list of documents and downloads each one. Note: This uses an undocumented endpoint and may not work for all notices.',
        inputSchema: {
          type: 'object',
          properties: {
            noticeId: {
            ...
            includeContent: {
              type: 'boolean',
              description: 'Whether to include the actual file content in the response (default: false). If true, files will be base64 encoded.',
              default: false,
            },
          },
          required: ['noticeId'],
  ...
  private async handleToolCall(name: string, args: any): Promise<any> {
    ...
    switch (name) {
      ...
      case 'download_all_documents':
        return await this.handleDownloadAllDocuments(args);
  ...
  private async handleDownloadAllDocuments(args: any) {
    if (!args.noticeId) {
      throw new Error('noticeId is required');
    }
    ...
    Logger.info(`Downloading all documents for notice: ${args.noticeId}`);
    ...
    Logger.info(`Found ${documents.length} documents to download`);

    // Download each document
    const downloadResults = [];
    for (const doc of documents) {
      try {
        Logger.info(`Downloading document: ${doc.name}`);

        if (includeContent) {
          const { content, contentType } = await this.apiClient.downloadDocumentFile(doc.url);
        ...
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        Logger.error(`Failed to download document ${doc.name}: ${errorMessage}`);
        downloadResults.push({
          id: doc.id,
          name: doc.name,
          type: doc.type,
    ...
    const documentSummary = downloadResults.map((result) => {
      ...
      if (result.size) {
        parts.push(`  - Size: ${(result.size / 1024).toFixed(2)} KB`);
      }

      if (result.uploadedDate) {
        parts.push(`  - Uploaded: ${result.uploadedDate}`);
      }

      if (result.status === 'success' && result.contentType) {
      ...
      if (includeContent && result.status === 'success' && result.content) {
        const contentPreview = result.content.length > 100
          ? `${result.content.substring(0, 100)}...`
          : result.content;
        parts.push(`  - Content: [${result.content.length} bytes]`);
      }
    ...
    const summary = [
      `# Downloaded Documents for Notice ${args.noticeId}`,
      ``,
    ...
    // If content was included, also return it as JSON for programmatic access
    if (includeContent) {
      const jsonData = JSON.stringify(downloadResults, null, 2);
      return {
        content: [
          {
            type: 'text',
